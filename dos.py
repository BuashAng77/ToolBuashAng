...
print("\033[1;31m!!WHEN ENTERING WEBSITE, MAKE SURE TO INCLUDE https://")
host = input(Fore.MAGENTA + "[</>] URL ATTACK WEBSITE: " + Style.RESET_ALL)
thread = int(input(Fore.MAGENTA + "[</>] ENTER THREAD ATTACK WEBSITE (1000/150000): " + Style.RESET_ALL))

# Biến để dừng mô phỏng
stop_simulation = False

def simulate_ddos():
    while not stop_simulation:
        # Mô phỏng tiêu đề HTTP
        headers = {
            "User-Agent": user_agents[threading.current_thread().ident % len(user_agents)],
            "Connection": "keep-alive"
        }
        try:
            # Mô phỏng yêu cầu với độ trễ ngẫu nhiên và mã trạng thái
            time.sleep(random.uniform(0.05, 0.3))  # Tốc độ nhanh để hiển thị liên tục
            status = random.choice([200, 403, 429, 503, "TIMEOUT"])
            # In dòng trạng thái
            thread_id = threading.current_thread().ident
            message = f"[{thread_id}] ---> Attack To Mock Server: [{host}] | Status: [{status}]"
            if status == 200:
                color = Fore.GREEN
            elif status == "TIMEOUT":
                color = Fore.RED
            ...
            pass
        time.sleep(0.05)  # Tốc độ cuộn

# Hàm tạo hiệu ứng cuộn từ trên xuống
def scroll_effect():
    while not stop_simulation:
        time.sleep(0.1)  # Tốc độ xóa màn hình
        # Xóa màn hình (Windows: cls, Unix: clear)
        os.system('cls' if os.name == 'nt' else 'clear')
        print(Fore.RED + "=== HACKER ATTACK SIMULATION ===" + Style.RESET_ALL)
        time.sleep(0.1)

# Bắt đầu mô phỏng
print(Fore.CYAN + "Starting attack simulation..." + Style.RESET_ALL)
time.sleep(1)

# Tạo và chạy luồng
thread_list = []
for i in range(thread):
    thread = threading.Thread(target=simulate_ddos)
    thread_list.append(thread)
    thread.start()
    time.sleep(0.01)

# Chạy mô phỏng trong 5 giây
time.sleep(5)
...
print(Fore.CYAN + "Simulation completed!" + Style.RESET_ALL)